...
=============================================*/
function preload() {
  audio = loadSound("Audio/Where_you_are.wav");
...
function draw() {
  ...
  color_index =
    isNaN(parseInt(decibel)) || decibel < -60
      ? (color_index = 0)
      : parseInt(map(decibel, -60, 0, 0, colorPalette.length - 1));

  var rev_color_index = map(
    color_index,
    0,
  ...
  for (i = 0; i < spectrogram.length; i += 1) {
    ...
    push();
    height_change = isNaN(spectrogram[i])
      ? (height_change = 1)
      : map(spectrogram[i], 0, 255, 5, 80);
